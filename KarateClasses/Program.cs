...
{
    options.Conventions.AuthorizeFolder("/Karate");
    options.Conventions.AllowAnonymousToPage("/Karate/Index");
    options.Conventions.AllowAnonymousToPage("/Karate/Details");
...
builder.Services.AddDbContext<KarateClassesContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("KarateClassesDB") ?? throw new InvalidOperationException("Connection string 'TrainingStudio02Context' not found.")));

builder.Services.AddDbContext<KarateClassesIdentityContext>(options =>
...
builder.Services.AddDefaultIdentity<IdentityUser>(options => options.SignIn.RequireConfirmedAccount = true)
    ...
    .AddEntityFrameworkStores<KarateClassesIdentityContext>();
